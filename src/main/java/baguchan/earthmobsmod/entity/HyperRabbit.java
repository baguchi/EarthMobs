...
public class HyperRabbit extends Rabbit {
	...
	public static AttributeSupplier.Builder createAttributes() {
		return Rabbit.createAttributes().add(Attributes.MAX_HEALTH, 4.0D).add(Attributes.MOVEMENT_SPEED, (double) 0.32F).add(Attributes.ATTACK_DAMAGE, 3.0);
	}
	...
	protected void dealDamage(LivingEntity livingentity) {
		...
		if (this.isAlive() && isSpark() && !this.isAlliedTo(livingentity) && this.level() instanceof ServerLevel serverLevel) {
			...
			if (livingentity.hurtOrSimulate(this.damageSources().indirectMagic(this, this), 2.0F * i)) {
				...
				livingentity.knockback(blocked * f1, d1, d2, this.damageSources().indirectMagic(this, this), 2.0F * i);
			}
		}
	}
