...
public class EntityLootTables extends EntityLootSubProvider {
    ...
    public void generate() {
        ...
        this.add(ModEntities.HORNED_SHEEP.get(), LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).add(LootItem.lootTableItem(Items.MUTTON).apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 2.0F))).apply(SmeltItemFunction.smelted().when(this.shouldSmeltLoot())).apply(EnchantedCountIncreaseFunction.lootingMultiplier(this.registries, UniformGenerator.between(0.0F, 1.0F))))).withPool(createSheepDispatchPool(BuiltInLootTables.SHEEP_BY_DYE)));
        LootData.WOOL_ITEM_BY_DYE.forEach((dye, wool) -> this.add(ModEntities.HORNED_SHEEP.get(), BuiltInLootTables.SHEEP_BY_DYE.get(dye), LootTable.lootTable().withPool(LootPool.lootPool().add(LootItem.lootTableItem(wool)))));
        ...
        this.add(ModEntities.WOOLY_COW.get(),
                LootTable.lootTable()
                        ...
                        ).withPool(createWoolyCowDispatchPool(Items.BROWN_WOOL)));
        this.add(ModEntities.UMBRA_COW.get(),
                LootTable.lootTable()
                        ...
                        ).withPool(createWoolyCowDispatchPool(Items.BLACK_WOOL)));
    ...
    public static LootPool.Builder createWoolyCowDispatchPool(Item item) {
        ...
        return LootPool.lootPool().add(LootItem.lootTableItem(item)).when(LootItemEntityPropertyCondition.hasProperties(LootContext.EntityTarget.THIS, EntityPredicate.Builder.entity().subPredicate(WoolyCowPredicate.hasWool())));
    }
    ...
    public static LootPool.Builder createSheepDispatchPool(Map<DyeColor, ResourceKey<LootTable>> tableNames) {
        ...
        for (Map.Entry<DyeColor, ResourceKey<LootTable>> e : tableNames.entrySet()) {
            variants = variants.otherwise(NestedLootTable.lootTableReference(e.getValue()).when(LootItemEntityPropertyCondition.hasProperties(LootContext.EntityTarget.THIS, EntityPredicate.Builder.entity().components(net.minecraft.advancements.criterion.DataComponentMatchers.Builder.components().exact(DataComponentExactPredicate.expect(DataComponents.SHEEP_COLOR, e.getKey())).build()).subPredicate(SheepPredicate.hasWool()))));
        }
